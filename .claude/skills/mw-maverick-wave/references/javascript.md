...
| FAQ                 | Opens the `mw-faq-item` a link points at - on load, on `hashchange` and on a same-page anchor click. Opening and closing is the `<details>` itself                                                                                                                               | Nothing - bind `[open]` if the app decides what is open                                                                                                                                                                                |
...
| Device frames       | On a `button.mw-device` with a screenshot: a tap sets `mw-active` and `aria-pressed`, and once the picture reached its end (`transitionend` on `translate`) a 1.2s pause and both come off again; a second tap ends the tour at once. The hover tour on the laptop is CSS        | Toggle `mw-active` on tap and clear it on `transitionend` plus a pause; skip a screenshot no taller than the glass                                                                                                                     |
