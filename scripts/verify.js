...
// Keyframes carry the same prefix and get named in the docs, but they are not
// classes - and one the docs name without the CSS having it still has to fail.
const keyframesInCss = new Set(all(css, /@keyframes\s+(mw-[a-z0-9-]+)/g));
...
for (const [c, where] of [...classesInMarkup].sort()) {
  if (
    classesInCss.has(c) ||
    classesInJs.has(c) ||
    keyframesInCss.has(c) ||
    CLASS_ALLOWLIST.has(c)
  ) {
    continue;
  }
